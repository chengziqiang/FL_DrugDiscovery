...
MODEL_META_NAME = "ECFPNNModelMeta"
MODEL_PARAM_NAME = "ECFPNNModelParam"
...
class ECFPNNBase(ModelBase):

    def __init__(self):
        super().__init__()
        self.model_param = ECFPNNParam()
        self.role = None
    ...
    def _init_model(self, param):
        ...
        self.transfer_variable = ECFPTransferVariable()
...
class ECFPNNArbiter(ECFPNNBase):
    ...
    def _init_model(self, param):
        super(ECFPNNArbiter, self)._init_model(param)
...
class ECFPNNClient(ECFPNNBase):

    def __init__(self):
        super().__init__()

    def _init_model(self, param):
        super(ECFPNNClient, self)._init_model(param)
        ...
        self.task_type = param.task_type
    ...
    def fit(self, data_inst, *args):
        data = self.data_converter.convert(data_inst, batch_size=self.batch_size,task_type=self.task_type) #change
        data_val = self.data_converter.convert(args[0],batch_size=-1,task_type=self.task_type)
        best = 9e8
        if not os.path.exists('/fate/model/{}'.format(self.taskid.split('_')[0])):
            os.makedirs('/fate/model/{}'.format(self.taskid.split('_')[0]))
        ...
        while self.aggregator_iter < self.max_iter:
            ...
            metrics_val = self.nn_model.evaluate(data_val)  #change
            Logger.info(f"valuation metrics at iter {self.aggregator_iter}: {metrics_val}")
            if best > metrics_val['mean_absolute_error']:
                self.nn_model._model.save('/fate/model/{}/{}_{}.h5'.format(self.taskid.split('_')[0],
                                                                           self.role,self.tracker.party_id))
...
class ECFPNNGuest(ECFPNNClient):
