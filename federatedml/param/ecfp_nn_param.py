...
class ECFPNNParam(BaseParam):
    ...
    def __init__(self,
                 ...
                 task_type: str = 'regression'):   #change

        super(ECFPNNParam, self).__init__()
        ...
        self.task_type = task_type   #change

    def check(self):
        supported_config_type = ["nn", "keras"]
        if self.config_type not in supported_config_type:
            raise ValueError(f"config_type should be one of {supported_config_type}")
        supported_task_type = ["regression", "classification"]
        if self.task_type not in supported_task_type:   #change
            raise ValueError(f"task_type should be one of {supported_task_type}")
    ...
    def generate_pb(self):
        pb = ecfp_nn_model_meta_pb2.ECFPNNParam()
        ...
        pb.loss = self.loss
        pb.task_type = self.task_type   #change
    ...
    def restore_from_pb(self, pb):
        ...
        self.loss = pb.loss
        self.task_type = pb.task_type   #change
